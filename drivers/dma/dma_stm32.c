...
static void dma_stm32_irq_handler(struct device *dev, uint32_t id)
{
	const struct dma_stm32_config *config = dev->config;
	DMA_TypeDef *dma = (DMA_TypeDef *)(config->base);
...
#ifdef DMA_STM32_SHARED_IRQS

static void dma_stm32_shared_irq_handler(void *arg)
{
...
DMA_STM32_EXPORT_DMAMUX int dma_stm32_configure(struct device *dev, uint32_t id,
...
DMA_STM32_EXPORT_DMAMUX int dma_stm32_reload(struct device *dev, uint32_t id,
...
DMA_STM32_EXPORT_DMAMUX int dma_stm32_start(struct device *dev, uint32_t id)
{
	const struct dma_stm32_config *config = dev->config;
	DMA_TypeDef *dma = (DMA_TypeDef *)(config->base);
...
DMA_STM32_EXPORT_DMAMUX int dma_stm32_stop(struct device *dev, uint32_t id)
{
	const struct dma_stm32_config *config = dev->config;
	struct dma_stm32_stream *stream = &config->streams[id - STREAM_OFFSET];
	DMA_TypeDef *dma = (DMA_TypeDef *)(config->base);
...
#else /* DMA_STM32_SHARED_IRQS */

#define DMA_STM32_DEFINE_IRQ_HANDLER(dma, chan)				\
static void dma_stm32_irq_##dma##_##chan(void *arg)			\
{									\
