...
/** @file
 * @brief I2C bus (TWIM) driver for Atmel SAM MCU family.
 *
...
#define DEV_CFG(dev) \
	((const struct i2c_sam_twim_dev_cfg *const)(dev)->config->config_info)
#define DEV_DATA(dev) \
	((struct i2c_sam_twim_dev_data *const)(dev)->driver_data)

static int i2c_clk_set(Twim *const twim, u32_t speed)
{
	u32_t ck_div = 0U;
	u32_t cl_div;
	...
	twim->TWIHS_CWGR = TWIHS_CWGR_CLDIV(cl_div) | TWIHS_CWGR_CHDIV(cl_div)
			    | TWIHS_CWGR_CKDIV(ck_div);

	return 0;
}

static int i2c_sam_twim_configure(struct device *dev, u32_t config)
{
	const struct i2c_sam_twim_dev_cfg *const dev_cfg = DEV_CFG(dev);
	Twim *const twim = dev_cfg->regs;
	u32_t bitrate;
	int ret;
	...
	ret = i2c_clk_set(twim, bitrate);
	if (ret < 0) {
		return ret;
	}

	/* Disable Slave Mode */
	twim->TWIHS_CR = TWIHS_CR_SVDIS;

	/* Enable Master Mode */
	twim->TWIHS_CR = TWIHS_CR_MSEN;

	return 0;
}

static void write_msg_start(Twim *const twim, struct twim_msg *msg,
			    u8_t daddr)
{
	/* Set slave address. */
	twim->TWIHS_MMR = TWIHS_MMR_DADR(daddr);

	/* Write first data byte on I2C bus */
	twim->TWIHS_THR = msg->buf[msg->idx++];

	/* Enable Transmit Ready and Transmission Completed interrupts */
	twim->TWIHS_IER = TWIHS_IER_TXRDY | TWIHS_IER_TXCOMP | TWIHS_IER_NACK;
}

static void read_msg_start(Twim *const twim, struct twim_msg *msg,
			   u8_t daddr)
{
	u32_t twim_cr_stop;

	/* Set slave address and number of internal address bytes */
	twim->TWIHS_MMR = TWIHS_MMR_MREAD | TWIHS_MMR_DADR(daddr);

	/* Enable Receive Ready and Transmission Completed interrupts */
	twim->TWIHS_IER = TWIHS_IER_RXRDY | TWIHS_IER_TXCOMP | TWIHS_IER_NACK;

	/* In single data byte read the START and STOP must both be set */
	twim_cr_stop = (msg->len == 1U) ? TWIHS_CR_STOP : 0;
	/* Start the transfer by sending START condition */
	twim->TWIHS_CR = TWIHS_CR_START | twim_cr_stop;
}

static int i2c_sam_twim_transfer(struct device *dev, struct i2c_msg *msgs,
			      u8_t num_msgs, u16_t addr)
{
	const struct i2c_sam_twim_dev_cfg *const dev_cfg = DEV_CFG(dev);
	struct i2c_sam_twim_dev_data *const dev_data = DEV_DATA(dev);
	Twim *const twim = dev_cfg->regs;
	...
	for (int i = 0; i < num_msgs; i++) {
		...
		dev_data->msg.idx = 0U;
		dev_data->msg.twim_sr = 0U;
		dev_data->msg.flags = msgs[i].flags;
		if ((msgs[i].flags & I2C_MSG_RW_MASK) == I2C_MSG_READ) {
			read_msg_start(twim, &dev_data->msg, addr);
		} else {
			write_msg_start(twim, &dev_data->msg, addr);
		}
		/* Wait for the transfer to complete */
		k_sem_take(&dev_data->sem, K_FOREVER);

		if (dev_data->msg.twim_sr > 0) {
...
static void i2c_sam_twim_isr(void *arg)
{
	struct device *dev = (struct device *)arg;
	const struct i2c_sam_twim_dev_cfg *const dev_cfg = DEV_CFG(dev);
	struct i2c_sam_twim_dev_data *const dev_data = DEV_DATA(dev);
	Twim *const twim = dev_cfg->regs;
	struct twim_msg *msg = &dev_data->msg;
	u32_t isr_status;

	/* Retrieve interrupt status */
	isr_status = twim->TWIHS_SR & twim->TWIHS_IMR;

	/* Not Acknowledged */
	if (isr_status & TWIHS_SR_NACK) {
		msg->twim_sr = isr_status;
		goto tx_comp;
	}
	...
	if (isr_status & TWIHS_SR_RXRDY) {
		...
		msg->buf[msg->idx++] = twim->TWIHS_RHR;

		if (msg->idx == msg->len - 1U) {
			/* Send STOP condition */
			twim->TWIHS_CR = TWIHS_CR_STOP;
		}
	}
	...
	if (isr_status & TWIHS_SR_TXRDY) {
		if (msg->idx == msg->len) {
			if (msg->flags & I2C_MSG_STOP) {
				...
				twim->TWIHS_IDR = TWIHS_IDR_TXRDY;
			} else {
...
tx_comp:
	...
	twim->TWIHS_IDR = twim->TWIHS_IMR;
	/* We are done */
	k_sem_give(&dev_data->sem);
}

static int i2c_sam_twim_initialize(struct device *dev)
{
	const struct i2c_sam_twim_dev_cfg *const dev_cfg = DEV_CFG(dev);
	struct i2c_sam_twim_dev_data *const dev_data = DEV_DATA(dev);
	Twim *const twim = dev_cfg->regs;
	...
	ret = i2c_sam_twim_configure(dev, bitrate_cfg);
	if (ret < 0) {
...
{
	IRQ_CONNECT(DT_I2C_0_IRQ, DT_I2C_0_IRQ_PRI, i2c_sam_twim_isr,
		    DEVICE_GET(i2c0_sam), 0);
}

static const struct soc_gpio_pin pins_twim0[] = PINS_TWIHS0;

static const struct i2c_sam_twim_dev_cfg i2c0_sam_config = {
	.regs = (Twim *)DT_I2C_0_BASE_ADDRESS,
	.irq_config = i2c0_sam_irq_config,
	.periph_id = DT_I2C_0_PERIPHERAL_ID,
	.irq_id = DT_I2C_0_IRQ,
	.pin_list = pins_twim0,
	.pin_list_size = ARRAY_SIZE(pins_twim0),
	.bitrate = DT_I2C_0_BITRATE,
};

static struct i2c_sam_twim_dev_data i2c0_sam_data;

DEVICE_AND_API_INIT(i2c0_sam, DT_I2C_0_NAME, &i2c_sam_twim_initialize,
		    &i2c0_sam_data, &i2c0_sam_config, POST_KERNEL,
		    CONFIG_I2C_INIT_PRIORITY, &i2c_sam_twim_driver_api);
#endif
