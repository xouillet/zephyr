...
struct i2c_sam_twim_dev_cfg {
	...
	uint8_t std_clk_strenght_low;
	uint8_t std_data_slew_lim;
	uint8_t std_data_strenght_low;
	uint8_t hs_clk_slew_lim;
	uint8_t hs_clk_strenght_high;
	uint8_t hs_clk_strenght_low;
	uint8_t hs_data_slew_lim;
	uint8_t hs_data_strenght_low;
...
#define DEV_CFG(dev) \
	((const struct i2c_sam_twim_dev_cfg *const)(dev)->config_info)
#define DEV_DATA(dev) \
	((struct i2c_sam_twim_dev_data *const)(dev)->driver_data)

static int i2c_clk_set(struct device *dev, uint32_t speed)
{
	const struct i2c_sam_twim_dev_cfg *const cfg = DEV_CFG(dev);
	Twim *const twim = cfg->regs;
	uint32_t per_clk = soc_pmc_peripheral_get_clock_hz(cfg->periph_id);
	...
	twim->SRR = ((speed == I2C_SPEED_FAST_PLUS) ? TWIM_SRR_FILTER(2) :
						      TWIM_SRR_FILTER(3)) |
		     TWIM_SRR_CLSLEW(cfg->std_clk_slew_lim) |
		     TWIM_SRR_CLDRIVEL(cfg->std_clk_strenght_low) |
		     TWIM_SRR_DASLEW(cfg->std_data_slew_lim) |
		     TWIM_SRR_DADRIVEL(cfg->std_data_strenght_low);
	twim->HSSRR = TWIM_HSSRR_FILTER(1) |
		      TWIM_HSSRR_CLSLEW(cfg->hs_clk_slew_lim) |
		      TWIM_HSSRR_CLDRIVEH(cfg->hs_clk_strenght_high) |
		      TWIM_HSSRR_CLDRIVEL(cfg->hs_clk_strenght_low) |
		      TWIM_HSSRR_DASLEW(cfg->hs_data_slew_lim) |
		      TWIM_HSSRR_DADRIVEL(cfg->hs_data_strenght_low);

	return 0;
}

static int i2c_sam_twim_configure(struct device *dev, uint32_t config)
{
	uint32_t bitrate;
	int ret;
	...
	case I2C_SPEED_FAST_PLUS:
		...
		break;
	case I2C_SPEED_HIGH:
		LOG_ERR("API not I2C speed value");
		return -EIO;
	default:
				     ...
				     uint32_t next_msg_idx)
{
	...
	*cmdr_reg &= (~(TWIM_CMDR_NBYTES_Msk |
	...
	if (data->next_need_rs) {
		// TODO: evaluate 10 bits repeat start read because of blank cmd
...
static void i2c_start_xfer(struct device *dev, uint16_t daddr)
{
	const struct i2c_sam_twim_dev_cfg *const cfg = DEV_CFG(dev);
...
#define I2C_TWIM_SAM_SLEW_REGS(n)					\
	.std_clk_slew_lim = DT_INST_PROP(n, std_clk_slew_lim),		\
	.std_clk_strenght_low = DT_INST_PROP(n, std_clk_strenght_low),	\
	.std_data_slew_lim = DT_INST_PROP(n, std_data_slew_lim),	\
	.std_data_strenght_low = DT_INST_PROP(n, std_data_strenght_low),\
	.hs_clk_slew_lim = DT_INST_PROP(n, hs_clk_slew_lim),		\
	.hs_clk_strenght_high = DT_INST_PROP(n, hs_clk_strenght_high),	\
	.hs_clk_strenght_low = DT_INST_PROP(n, hs_clk_strenght_low),	\
	.hs_data_slew_lim = DT_INST_PROP(n, hs_data_slew_lim),		\
	.hs_data_strenght_low = DT_INST_PROP(n, hs_data_strenght_low)
...
#define I2C_TWIM_SAM_INIT(n)						\
	...
	static const struct i2c_sam_twim_dev_cfg i2c##n##_sam_config = {\
		...
		.hs_master_code = DT_INST_PROP(n, hs_master_code),	\
