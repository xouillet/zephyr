...
#define DIV_MAX    15
	...
	vco_hz = DT_PROP(SOC_ATMEL_PLLCLK_NODE, clock_frequency) * mul;
	vco_hz /= divide;
...
/*#if DT_HAS_NODE(SOC_ATMEL_PLLCLK_NODE)*/
	/* Enable PLL */
	if (!pll_is_locked(0)) {
		...
		uint32_t pll_config = pll_config_init(
				   DT_PROP(SOC_ATMEL_PLLCLK_NODE, pll_div),
				   DT_PROP(SOC_ATMEL_PLLCLK_NODE, pll_mul));
...
/*#endif*/
	...
	PM->PBASEL = PM_PBASEL_PBDIV | PM_PBASEL_PBSEL(2);
	...
	PM->PBCSEL = PM_PBCSEL_PBDIV | PM_PBCSEL_PBSEL(3);

	PM->UNLOCK = PM_UNLOCK_KEY(0xAAu) |
		     PM_UNLOCK_ADDR((uint32_t)&PM->PBDSEL - (uint32_t)PM);
	PM->PBDSEL = PM_PBDSEL_PBDIV | PM_PBDSEL_PBSEL(3);
